...
# This is a modification to Scikit's t-SNE implementation
...
import pyximport ##GUY
pyximport.install() ##GUY
...
def _kl_divergence_modi(params, P, degrees_of_freedom, n_samples, n_components,
                   skip_num_points=0, compute_error=True, alpha=0, Q_zero=None ):
    """t-SNE objective function: gradient of the KL divergence
    of p_ijs and q_ijs and the absolute error.
    Parameters
    ----------
    ...
    # Objective: C (Kullback-Leibler divergence of P and Q)
    if compute_error: ##GUY
        kl_divergence_P = 2.0 * np.dot(
            P, np.log(np.maximum(P, MACHINE_EPSILON) / Q))
        if (Q_zero is None):
            kl_divergence_Q=np.zeros_like(kl_divergence_P)
        else:
            kl_divergence_Q = 2.0 * np.dot(
                Q_zero, np.log(np.maximum(Q_zero, MACHINE_EPSILON) / Q))
        kl_divergence=(1-alpha)*kl_divergence_P+alpha*kl_divergence_Q
    else:
        kl_divergence = np.nan
    ...
    # pdist always returns double precision distances. Thus we need to take
    grad = np.ndarray((n_samples, n_components), dtype=params.dtype)
    PQd = squareform(((1-alpha)*P+alpha*Q_zero - Q) * dist)##GUY
    for i in range(skip_num_points, n_samples):
        grad[i] = np.dot(np.ravel(PQd[i], order='K'),
                         X_embedded[i] - X_embedded)
    grad = grad.ravel()
...
class TSNE(BaseEstimator):
    ...
    def _tsne(self, P, degrees_of_freedom, n_samples, X_embedded,
        ...
        if self.method == 'barnes_hut':
            ...
            obj_func = _kl_divergence_modi

        # Learning schedule (part 1): do 250 iteration with lower momentum but
        # higher learning rate controlled via the early exageration parameter
        P *= self.early_exaggeration
        if ((obj_func==_kl_divergence_modi) and not (self.Q_zero is None)):##GUY
            params, kl_divergence, it, Q_zero = _gradient_descent(obj_func, params,
                                                          **opt_args)
            self.Q_zero=Q_zero
        else:
